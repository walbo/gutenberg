 ...
 */
function render_nested_page_list( $nested_pages, $active_page_ancestor_ids = array(), $colors, $depth = 0 ) {
	...
	foreach ( (array) $nested_pages as $page ) {
		...
		if ( isset( $page['children'] ) ) {
			...
			$markup .= '<ul class="submenu-container">' . render_nested_page_list( $page['children'], $active_page_ancestor_ids, $colors, $depth + 1 ) . '</ul>';
		}
		$markup .= '</li>';
	}
	return $markup;
...
function render_block_core_page_list( $attributes, $content, $block ) {
	...
	$items_markup = render_nested_page_list( $nested_pages, $active_page_ancestor_ids, $colors );
